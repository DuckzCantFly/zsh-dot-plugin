...
if [ -x "$(command -v lsb_release)" ] ; then
	LOCAL_SYS_DISTRO_ID="$(lsb_release -a | grep -e "Distributor ID " | cut -d ':' -f2 | tr -d '[:space:]')"
	return
elif 
	if ls -U /etc/*release 1>& /dev/null 2>&1 ;then
		LOCAL_SYS_DISTRO_ID="$(cat /etc/*release | grep -m 1 "ID=" | cut -d '=' -f2 | tr -d '[:space:]')"
	else 
		[ 1 = 2 ]
...
then ; return
	...
	if ls -U /etc/issue* 1>& /dev/null ; then
		LOCAL_SYS_DISTRO_ID="$(cat /etc/issue* | cut -d ' ' -f1 | tr -d '[:space:]')"
	else
		[ 1 = 2 ]
	fi ; [ -n "${LOCAL_SYS_DISTRO_ID}" ]
then ; return
